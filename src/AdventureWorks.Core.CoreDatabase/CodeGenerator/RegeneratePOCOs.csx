...
string inputJsonSchema = Path.GetFullPath(Path.Combine(GetScriptFolder(), jsonSchemaPath));
string targetFolder = Path.GetFullPath(Path.Combine(GetScriptFolder(), targetFolderPath));


var generator = new SimplePOCOGenerator(inputJsonSchema);
generator.Namespace = entitiesNamespace;
generator.CrudClassName = crudClassName;
generator.CrudClassNamespace = crudClassNamespace;
generator.CrudClassFile = crudClassFile;
